...
cd PromiseLink
pip install -e '.[dev]'
pytest --co -q | tail -1   # 应显示 1394 tests collected
...
| 指标       | 数值                                                               |
| -------- | ---------------------------------------------------------------- |
| 测试用例     | **1364 passed**, 30 skipped, 0 failed (含 50 个 relay_client 健壮性 + 12 个 v5.6 纠偏 + 50 安全 + 17 性能 + 6 真实 LLM E2E) |
| 代码覆盖率    | **71%**                                                          |
...
- [x] 67 个测试文件 / **1394 测试用例**（含 50 个 relay_client 健壮性 + 12 个 v5.6 纠偏 + 6 真实 LLM E2E）/ **71% 覆盖率**
- [x] CI/CD + Alembic 就绪
- [x] PoC Demo 4/4 场景通过
