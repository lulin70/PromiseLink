...
<p align="center">
  ...
  <a href="https://github.com/lulin70/PromiseLink/releases"><img src="https://img.shields.io/badge/version-v0.9.9-blue?style=flat-square" alt="Version"></a>
  <br/>
...
| 🏭 **工业级质量** | 1922 测试通过 / 89% 覆盖率 / mypy 0 / ruff 0 / 50 安全测试 / 17 性能测试 | 多数开源 CRM < 30% 覆盖率 |
...
**v0.9.9 直接下载**（双击安装，无需 Python 环境）：

- **macOS**: [PromiseLink-0.9.9-mac.dmg](https://github.com/lulin70/PromiseLink/releases/download/v0.9.9/PromiseLink-0.9.9-mac.dmg)（Apple Silicon / Intel，~82 MB）
- **Windows**: [PromiseLink-0.9.9-windows.exe](https://github.com/lulin70/PromiseLink/releases/download/v0.9.9/PromiseLink-0.9.9-windows.exe)（Windows 10/11 64-bit，~42 MB）
...
| 指标       | 数值                                                               |
| -------- | ---------------------------------------------------------------- |
| 测试用例     | **1922 passed**, 49 skipped, 0 failed (含 50 个 relay_client 健壮性 + 12 个 v5.6 纠偏 + 50 安全 + 17 性能 + 6 真实 LLM E2E) |
| 代码覆盖率    | **89%**                                                          |
...
> 提供具有舆论属性或者社会动员能力的生成式人工智能服务的，应当按照国家有关规定开展安全评估，并**按照《互联网信息服务深度合成管理规定》履行算法备案和变更、注销备案手续**。

以及《生成式人工智能服务安全基本要求》（TC260-003 2024）关于日志留存的要求，PromiseLink 在专业版网关侧实现了**国内大模型对话日志 6 个月以上留存**机制：

- **存储位置**：专业版网关 PostgreSQL 数据库 `llm_chat_logs` 表
- **留存周期**：≥ 180 天（6 个月）
- **日志内容**：用户 ID（脱敏）、时间戳、请求内容、响应内容、模型、Token 用量、请求 ID
- **自动清理**：`scripts/cleanup_chat_logs.py` 按 retention_days 配置自动清理过期日志
- **基础版责任**：基础版本地不存储 LLM 对话原文，所有 LLM 调用通过专业版网关中继（`relay_client` → 网关 `/api/v1/pro/relay/llm`），日志在网关侧统一留存

详见专业版仓库 [PromiseLink-Pro](https://github.com/lulin70/PromiseLink-Pro) `docs/planning/LLM对话日志留存6个月方案_v1.md`。
