...
| B2 | RSS 追踪 → feedparser 定时拉取 → 关联人脉动态 | Agent-Reach rss.py | 1.5 周 | P0 |
...
### v0.9.5：多语言支持 + 跨语言实体关联（预估 3 周）

**目标**：支持中/英/日三语界面与内容录入，实现跨语言实体关联（同一主体不同语言描述能关联到同一实体）。

**典型场景**：用户拿到的会议纪要是英文的，自己文字录入是中文的，但讲的是同一件事 / 同一个人 —— 系统应能识别"John Smith"和"约翰·史密斯"是同一人脉，"follow up next week"和"下周跟进"是同一待办。
