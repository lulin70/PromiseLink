...
| 9 | EventLink→CarryMem写入映射表 | **采纳** | Protocol接口已定义，但语义映射缺失。建议补充：(1) 每种写入的schema定义（CarryMem期望的JSON结构）；(2) 写入失败的重试策略；(3) CarryMem未部署时的Mock数据存储（避免PoC期间写入丢失） |
...
| 5 | **CarryMem集成延迟** | CarryMem SDK更新慢，EventLink功能受阻 | (1) NullMemoryProvider必须功能完整（不依赖CarryMem也能运行）；(2) Protocol接口版本管理；(3) Mock数据生成器（测试用） |
...
| 9 | EventLink→CarryMem映射表 | **不采纳（PoC）** | PoC用NullMemoryProvider，不依赖CarryMem集成。Phase1集成时再补。 |
...
| 9 | EventLink→CarryMem写入映射表 | **采纳** | 表格清晰。补充：<br>1. declare_memory的payload schema需要CarryMem SDK文档<br>2. update_rule的rule格式需要明确（JSON？DSL？）<br>3. 建议加失败重试队列（CarryMem不可用时）<br>**估算：1天**（Phase2前） |
...
| 9 | EventLink→CarryMem映射表 | **不采纳（PoC）** | P2，PoC用NullMemoryProvider即可。Phase2集成测试时补，重点验证：①映射完整性 ②写入失败回滚 ③CarryMem侧数据一致性校验 |
...
| 9 | EventLink→CarryMem映射 | **修改采纳** | **关键安全风险**：跨系统写入需要明确权限边界。建议补充：1) declare_memory调用必须携带scoped token（仅能写入特定类型记忆）；2) CarryMem应验证写入来源（EventLink的service account）；3) 敏感决策（如"用户忽略某Todo"）应加签名防篡改 |
...
3. **具体方案**：
   ...
   db_key = os.environ.get("EVENTLINK_DB_KEY")  # 从环境变量读取
   if not db_key:
       raise RuntimeError("DB encryption key not set")
   
   engine = create_engine(
       f"sqlite+pysqlcipher://:{db_key}@/eventlink_poc.db"
   )
   ```
...
**问题**：EventLink通过declare_memory写入CarryMem，若权限控制不当，EventLink的漏洞可能污染CarryMem的全部用户数据。

**建议**：
```python
# EventLink调用CarryMem时使用scoped token
class CarryMemClient:
    ...
    async def declare_memory(self, user_id: str, entry: dict):
        ...
        if entry["type"] not in ["fact_declaration", "decision"]:
            raise PermissionError(f"EventLink cannot write {entry['type']}")
...
**权限矩阵**：
| EventLink产出 | CarryMem记忆类型 | EventLink是否可写 |
|--------------|-----------------|------------------|
...
1. **端到端加密**：离线数据用AES-256-GCM加密，密钥派生自用户PIN+设备指纹
   ...
   localStorage.setItem("eventlink_offline", encryptedData);
   ```
...
3. **自动过期**：离线数据TTL=7天，超期自动删除
   ```javascript
   const offlineData = JSON.parse(localStorage.getItem("eventlink_offline"));
   if (Date.now() - offlineData.timestamp > 7 * 24 * 60 * 60 * 1000) {
       localStorage.removeItem("eventlink_offline");
   }
   ```
...
| 9 | EventLink→CarryMem映射表 | **采纳** | 需要补，且需要版本化。如果CarryMem协议升级，旧版EventLink不能写坏数据 |
...
```bash
# 每日备份
0 2 * * * cp /data/eventlink.db /backup/eventlink_$(date +\%Y\%m\%d).db
...
# 全量备份（每日）
0 2 * * * pg_dump eventlink | gzip > /backup/eventlink_$(date +\%Y\%m\%d).sql.gz
...
```
单机Docker
├── eventlink-api (FastAPI)
├── eventlink-worker (Celery)
├── eventlink-scheduler (APScheduler)
└── eventlink.db (SQLite)
```
...
```
Docker Compose
├── eventlink-api (2副本 + Nginx LB)
├── eventlink-worker (2副本)
├── eventlink-scheduler (1副本)
├── postgres (主)
...
| 9 | EventLink→CarryMem写入映射表 | **不采纳**（P2保持） | 这是后台集成细节，不影响用户可见的交互 |
