...
|----------|----------|------------|----------|
| **专业版通过 HTTP API 调用基础版** | 网络通信，两进程独立 | ❌ 不传染 | 独立进程间通过 HTTP 通信，不构成"修改文件"，且 MPL 无网络条款 |
| **专业版网关通过 WSS 调用 relay_client** | 网络通信 | ❌ 不传染 | relay_client（基础版 MPL 文件）与网关（专业版私有）通过 WebSocket 通信，独立进程，不修改文件 |
| **专业版通过 pip install 引入基础版** | 包依赖 | ❌ 不传染 | import 调用不构成"修改文件"，基础版文件保持 MPL，专业版文件不受传染 |
...
## 5. 桥接接口不构成"衍生作品"的法律分析

### 5.1 API 桥接的隔离原理

PromiseLink 专业版通过 HTTP API 调用基础版，这种"桥接"（bridging）关系不触发 MPL copyleft，理由如下：

1. **进程隔离**：基础版（FastAPI 进程）与专业版（网关进程）是两个独立的操作系统进程，各自有独立的内存空间、进程 ID、生命周期。

2. **通信方式**：两进程通过标准 HTTP/HTTPS 协议通信，与浏览器访问 Web 服务器的技术本质相同。通过标准网络协议通信的程序之间不构成"修改"关系。

3. **文件级隔离**：MPL 2.0 的 copyleft 限定在文件级。专业版代码文件是独立的文件，不修改任何基础版 MPL 文件，因此专业版文件不受 MPL 传染。
...
5. **类比先例**：此模式类似于：
   - Firefox（MPL 2.0）调用闭源插件
   - Thunderbird（MPL 2.0）与闭源邮件服务通信
   - MongoDB（原 SSPL，源自 MPL）被闭源应用通过驱动调用
   这些场景中，Mozilla 和开源社区均认可进程隔离 + 网络通信的隔离边界。

### 5.2 专业版通过 API 调用基础版不触发 MPL 传染
...
**结论**：专业版通过 HTTP API 调用基础版，两进程独立运行，不修改任何 MPL 文件，不触发 MPL 2.0 传染。专业版可保持闭源商业许可。

### 5.3 pip install 依赖的隔离分析
...
- **基础版文件保持 MPL**：被 import 的基础版文件仍然是 MPL 许可，专业版分发时需包含基础版的 MPL 文件和许可声明
...
4. **不修改原则**：专业版通过 pip install 引入基础版，不修改任何基础版源码文件。
...
- [ ] HTTP API 调用 + pip import 是否完全不触发 MPL 2.0 传染
...
1. **AGPL v3 第 13 条的传染风险**：虽然技术团队评估认为 HTTP API 通信不触发传染，但 AGPL v3 的网络交互条款在法律实践中存在争议，商业用户普遍规避 AGPL。
