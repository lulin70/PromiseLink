...
| **专业版**（PromiseLink-Pro） | `lulin70/PromiseLink-Pro`（私有） | 私有商业许可 | 不受 MPL 约束 | 商业授权 |
...
- 基础版仓库：包含完整的 13 步管线、实体提取、记忆、Todo、Promise、关联发现、桌面包、H5、适配器
...
- **如需闭源使用**：可直接修改基础版新增闭源文件（MPL 允许 Larger Work），但修改过的 MPL 文件仍需开源。如需完全闭源，建议获取专业版商业授权。
...
- 包含桌面安装包一键安装、H5 宽屏前端、SQLite 存储、PIPL/GDPR 隐私合规
