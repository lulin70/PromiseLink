...
> **更新日期**: 2026-06-18
> **背景**: 基于当前单机 4核8G 部署架构，评估承载上限、扩容触发信号、LLM成本模型，为后续规模化提供明确方向。

#### 12.7.1 当前架构承载上限

**设计前提**: §1.4 明确"单机部署足够（<100用户）"，此处给出具体量化分析。

| 层面 | 当前设计上限 | 实际瓶颈在哪 |
|------|-------------|---------|
| Nginx 层 | 100 req/s per IP（限流配置） | 单机 Nginx 实际可承受 5000+ req/s，IP 维度不是瓶颈 |
| WebSocket 连接 | 50 连接/IP，42 个当前活跃 | Redis 存 ws_connections，单机 Redis 可撑 5 万连接 |
| 用户级限流 | 100 req/min per user | 每用户每分钟最多调 100 次 AI |
| VPS 规格 | **4核8G** | CPU 是实际瓶颈，Python async FastAPI 下约 200-500 并发请求 |
| PostgreSQL | 单机，无读写分离 | 高并发写（usage_records）时将成为瓶颈 |

**实际并发用户数换算**:

PromiseLink 不是即时通讯工具，用户使用模式为**低频、高价值**（每天几次 AI 调用，非持续在线）：

- 100 个活跃付费用户，每人每天用 10 次 AI，分散在 8 小时 → 平均 0.03 req/s
- 100 个用户同时在线的概率极低
- **实际峰值并发估计：15-25 req/s，4核8G 完全充裕**

**结论：当前架构支撑 300-500 付费用户无压力。超过 500 用户需关注以下扩容触发信号。**

#### 12.7.2 扩容触发信号（网关侧）

| 用户规模 | 建议动作 | 预估月收入（¥29） |
|----------|---------|-------------------|
| **< 500 用户** | **单机 4核8G，无需任何改动** | < ¥14,500/月 |
| **500-2000 用户** | 加一台网关备机 + Nginx upstream 轮询 | ¥14,500-58,000/月 |
| **2000-5000 用户** | 换 8核16G + Redis 独立部署 + PG 加读副本 | ¥58,000-145,000/月 |
| **> 5000 用户** | 上 K8s，Gateway 多实例，PG 分库分表 | > ¥145,000/月 |

> **说明**: 网关已设计为**无状态**（§1.2 可扩展原则），水平扩展只需加实例 + Nginx upstream 即可，无需改代码。

**具体触发指标**:

| 指标 | 阈值 | 动作 |
|------|------|------|
| CPU 持续使用率 | > 70% (5分钟) | 评估加实例 |
| P95 响应时间 | > 2.5s（设计目标见 §12.4） | 排查瓶颈，必要时扩容 |
| 每分钟请求数 | > 300 req/min | 加网关备机 |
| PG 连接数 | > 16（告警阈值见 §10.5.3） | 加连接池或读副本 |
| Redis 内存使用 | > 80% | 扩容 Redis 或独立部署 |

#### 12.7.3 LLM 侧负载均衡（已内置）

LLM 调用侧的"负载均衡"**已在 §5 API Key 池管理算法中完整覆盖**，无需额外开发：

| 能力 | 实现位置 | 说明 |
|------|---------|------|
| 多 Key 加权轮询 | §5.2 | 按 `base_weight × health_score × (1 - rpm/limit)` 选择 |
| 429 限流冷却 | §5.4 | 收到 429 → 冷却 60s → 自动切换其他 Key |
| 熔断恢复 | §5.5 | 连续 3 次 5xx → 熔断 5min → 探活恢复 |
| Provider 降级 | §8.4 | DeepSeek 不可用 → 自动切换 Moka AI |

**后续需要关注的 LLM 成本而非负载均衡**。

#### 12.7.4 Token 成本模型与定价校验

**当前配额**: 早鸟版每用户 50 万 Token/月（§7.1）。

| 场景 | 月度 Token 消耗 | LLM 成本（¥0.001/1K） | 月收入（¥29） | 毛利 |
|------|----------------|----------------------|--------------|------|
| 轻度用户（30% 配额） | 15 万 | ¥0.15 | ¥29 | ¥28.85 |
| 中度用户（60% 配额） | 30 万 | ¥0.30 | ¥29 | ¥28.70 |
| 重度用户（100% 配额） | 50 万 | ¥0.50 | ¥29 | ¥28.50 |
| 100 用户综合 | ~2500 万 | ¥25 | ¥2,900 | ¥2,875 |
| 500 用户综合 | ~1.25 亿 | ¥125 | ¥14,500 | ¥14,375 |
| 2000 用户综合 | ~5 亿 | ¥500 | ¥58,000 | ¥57,500 |

**结论**: LLM 成本在总收入中占比极低（< 1%），不是核心成本项。真正的成本是服务器和带宽。

**需要注意的风险**: 异常刷量。已在 §9.4.2 覆盖——单用户小时 Token > 10 万自动告警，> 20 万自动降级限流至 20 req/min。

#### 12.7.5 扩容路线图总结

```
Phase 0（当前）          Phase 1（500用户）        Phase 2（2000用户）       Phase 3（5000+用户）
┌──────────────┐       ┌──────────────┐        ┌──────────────┐         ┌──────────────┐
│ 单机 4核8G    │  ──→  │ +1台备机      │  ──→   │ 8核16G       │  ──→    │ K8s 集群     │
│ PG + Redis    │       │ Nginx upstream│        │ Redis 独立    │         │ PG 分库分表  │
│ 单实例        │       │ 网关无状态扩  │        │ PG 读写分离   │         │ 网关多副本   │
│ <500 用户     │       │ 500-2000用户  │        │ 2000-5000用户 │         │ >5000 用户   │
└──────────────┘       └──────────────┘        └──────────────┘         └──────────────┘

LLM 侧: Key池多路轮询 + 熔断冷却 + Provider降级（§5，已内置，无需随用户规模变更）
```
...
| 原则 | 当前落实 | 扩容后状态 |
|------|---------|-----------|
| **高可用** | Key池熔断+冷却恢复、PG备份、Redis AOF | 多实例冗余，单点故障自动切换 |
| **可扩展** | 网关无状态、Redis共享连接映射 | 加实例即可水平扩展，无需改架构 |
| **安全** | RS256签名+设备指纹+用户级限流 | 扩容后限流/CRL仍为全局一致（Redis共享状态） |
| **可观测** | Prometheus指标、结构化日志、健康检查 | 多实例时按 instance 标签区分，告警规则不变 |
...
| v1.2 | 2026-06-18 | 新增 §12.7 可扩展性分析与后续扩容方向：承载上限量化（300-500用户）、扩容触发信号、LLM成本模型、扩容路线图 |
...
*文档结束 | PromiseLink专业版Phase 0技术设计 v1.2 | P3技术设计阶段 | P6安全审查阻断项已修复 | 新增可扩展性分析*
