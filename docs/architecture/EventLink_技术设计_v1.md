...
4. **与CarryMem解耦** — EventLink是独立服务，通过协议接口消费CarryMem能力，不直接依赖其内部实现

### 1.2 与CarryMem的边界

```
CarryMem（AI记忆层）          EventLink（商务关系引擎）
├── 个人偏好/决策/纠正        ├── 事件/实体/关联/Todo
├── 规则引擎（forbid/avoid）  ├── 关联发现引擎
├── SQLite本地存储            ├── PostgreSQL + Redis
└── Python API + MCP          └── HTTP REST API

集成点（单向消费）：
EventLink → CarryMemAdapter → CarryMem.recall_memories()
                                CarryMem.match_rules()
                                CarryMem.declare()

不做的：
❌ EventLink不把CarryMem当存储层
❌ EventLink不调用CarryMem.classify_and_remember()
❌ CarryMem不感知EventLink的数据模型
```
...
│  ┌─────────────────────────────────────────────────────┐ │
│  │ EventLink H5页面（Phase 1，WebView内嵌）              │ │
...
┌──────────────────────────────────────────────────────────┐
│                    数据层                                   │
│  PostgreSQL 15 (Event/Entity/Association/Todo)            │
│  Redis 7 (缓存/会话/限流)                                  │
...
Phase 1（快速验证）：
  ...
  │   ├── wx.startRecord → ASR → EventLink API（语音录入）
  │   ├── 微信同声传译插件 → TTS播报（会前速览）
  │   └── 名片扫描 → 直接调用 POST /api/v1/events (card_save)
  └── WebView → EventLink H5页面（仅只读查询场景）
...
```javascript
// 方案：onShow生命周期 + Storage共享（Phase 1最简方案）
// 原生页面录音/操作完成后：
wx.setStorageSync('_eventlink_sync', {
  timestamp: Date.now(),
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') {
    const sync = wx.getStorageSync('_eventlink_sync')
    if (sync && sync.timestamp > lastSyncTime) {
      lastSyncTime = sync.timestamp
      wx.removeStorageSync('_eventlink_sync')
      refreshData(sync.entity_ids)  // 增量刷新受影响的实体
    }
  }
})

// Phase 2升级：EventChannel双向通信
...
class TTSCacheManager:
    ...
    # 缓存介质: Redis (Phase 1) → OSS+CDN (Phase 2)
    CACHE_TTL = 3600  # 1小时
...
**备选方案：EventLink自建小程序（Taro框架）【v2.3新增】**
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
CREATE TABLE entities (
    ...
    properties JSONB DEFAULT '{}',     -- 扩展画像存这里（含concern/promise/contribution + Phase2的resource/demand）
...
实现要点：
1. **存储前**：调用 `eventlink.core.text_utils.sanitize_llm_input()` 清洗注入风险
2. **存储时**：不建全文索引（避免敏感信息泄露到搜索结果）
3. **返回前**：API层调用新的 `redact_pii_from_text()` 函数脱敏
4. **导出时**：同样执行脱敏（CSV/JSON导出均适用）

新增工具函数位置：`src/eventlink/core/text_utils.py`
...
> - **resource/demand**字段保留但PoC阶段不启用，Phase2资源经营能力开放后使用
...
> **v2.0定位校准**：匹配逻辑从"资源匹配"转为"关系经营"。PoC先做承诺兑现闭环，不启用六维匹配；Phase1启用care维度；Phase2完整六维匹配。
...
#### 4.5.2 Phase1：启用care维度（关注点匹配）

Phase1在承诺闭环基础上，新增care维度匹配——当两个人的关注点有交集时，提示"值得关心"。
...
#### 4.5.3 Phase2：完整六维匹配（callability降级为参考维度）

Phase2启用完整六维匹配，但callability维度从20%降权为10%（因为利他逻辑下"可调用"不是首要目标），新增care_overlap维度10%。
...
def create_db_engine(stage: str = "poc"):
    if stage == "poc":
        db_path = os.environ.get("EVENTLINK_DB_PATH", "eventlink_poc.db")
        db_key = os.environ.get("EVENTLINK_DB_KEY")
        if db_key:
...
> **核心原则**：EventLink是个人商务关系经营助手，AI是辅助工具，不是决策者。AI的输出必须尊重用户对真实关系的掌控权，避免越界推断和冒犯性建议。
...
**关键**：EventLink的所有核心功能在`NullMemoryProvider`下依然可用，CarryMem只是增强。
...
#### 7.2 日视图API（F-49, Phase 1，v2.4新增）
...
| 方案 | 优点 | 缺点 | Phase |
|------|------|------|-------|
| 微信同声传译插件 | 小程序原生支持，免费 | 音质一般 | Phase 1 |
| 讯飞语音合成 | 音质好，中文优化 | 需付费 | Phase 2 |
| Azure TTS | 多语言，音质最佳 | 成本高 | Phase 2 |
...
| Repudiation | 否认操作 | 操作审计日志(Audit Logger) | 📋 Phase 1 |
...
Phase 1 补充：T(防篡改HTTPS) + R(操作审计日志) + D(完整Rate Limit)
...
sqlalchemy.url = postgresql://eventlink:***@localhost:5432/eventlink
...
def run_migrations_online():
    engine = create_engine(get_db_url())  # PoC=SQLite, Phase1=PG
...
3. **PoC→Phase1是一次性全量迁移**：不走Alembic增量，使用 `pg_dump` + 迁移脚本
...
```bash
# Step 1: 导出SQLite数据
sqlite3 eventlink_poc.db .dump > /tmp/eventlink_dump.sql
...
def get_logger(name: str = "eventlink") -> structlog.stdlib.BoundLogger:
    """获取带模块名的logger"""
...
```json
{
  ...
  "user_id": "usr_abc123",
  "module": "eventlink.services.entity_resolution",
...
loggers:
  eventlink:
    ...
    propagate: false
  eventlink.services.llm_client:
    level: INFO
    handlers: [console, file]
  eventlink.services.entity_resolution:
    level: DEBUG
    handlers: [console, file]
```
...
```python
# src/eventlink/core/exceptions.py
...
```python
# src/eventlink/core/error_handlers.py
from fastapi import Request
from fastapi.responses import JSONResponse
import structlog

logger = structlog.get_logger("eventlink.error_handler")

async def eventlink_error_handler(request: Request, exc: EventLinkError) -> JSONResponse:
    """EventLink业务异常统一处理"""
    logger.error(
        "business_error",
        code=exc.code,
...
> **对应PRD**: §1.5 部署模型与数据主权
> **核心决策**: PoC本地优先（零云成本+数据不出本机），Phase 1云端托管（规模化+集中运维），前端始终以微信小程序为主入口，不开发原生APP。

### 8.6.1 为什么不做原生APP

LLM推理必须走云端，手机只是展示层。微信小程序已覆盖EventLink全部核心交互（语音录入/ TTS播报/名片扫描/推送通知），原生APP无增量价值，且获客成本高（下载APP vs 扫码即用）、开发成本高（双端 vs 单端）。
...
services:
  eventlink-api:
    ...
    ports:
      - "8000:8000"
    environment:
      - EVENTLINK_STAGE=phase1
      - DATABASE_URL=postgresql://eventlink:${PG_PASSWORD}@postgres:5432/eventlink
...
| 数据可删除 | /api/v1/account/delete 级联删除+账号注销 | Phase 1 |
...
### 8.6.6 PoC→Phase 1迁移方案

**核心优势**：SQLAlchemy抽象层使数据库切换零代码改动。

```python
# 迁移仅需修改环境变量
# PoC: EVENTLINK_STAGE=poc → SQLite
# Phase1: EVENTLINK_STAGE=phase1 → PostgreSQL

# 数据迁移脚本
# sqlite3 eventlink_poc.db .dump | python3 scripts/migrate_sqlite_to_pg.py
```

| 迁移项 | PoC | Phase 1 | 改动量 |
|--------|-----|---------|--------|
| 数据库连接 | SQLite文件 | PG连接字符串 | 1行配置 |
...
6. **Person画像**：Phase 1维度（基础+沟通+决策+concern/promise/contribution+关系）
...
| 检查项 | 验证方法 | 通过标准 |
|--------|----------|----------|
| EventLink可独立启动 | 不安装CarryMem，启动EventLink API | 所有核心API可用 |
| NullMemoryProvider降级 | 配置memory_provider=null | 功能不缺失，仅缺少偏好增强 |
| CarryMem升级不影响EventLink | CarryMem API变更后 | EventLink仅需修改Adapter，核心逻辑不变 |
| EventLink数据模型独立 | 检查import | 无`from carrymem import`在核心模块中 |
| 可独立测试 | pytest without CarryMem | 测试覆盖率≥80% |

---

## 11. 技术启发：Understand-Anything对EventLink的架构借鉴
...
Understand-Anything处理的是**代码实体**（文件/函数/类）及其**依赖关系**（调用/继承/引用），EventLink处理的是**商务实体**（人物/组织/话题）及其**关联关系**（共现/合作/引荐）。两者本质都是**实体-关系图谱+AI语义增强**。
...
### 11.2 启发1：会前简报包（Phase 1实现）

**UA做法**：将代码库上下文打包为`.agentic`文件，AI工具加载后获得100%准确的项目上下文。

**EventLink借鉴**：会前简报包 = 目标人物的所有关联上下文一次生成、多处复用。
...
### 11.3 启发2：关系图谱交互设计（Phase 1 Plus实现）
...
**技术选型**：D3.js force-directed graph（Phase 1 H5）→ 小程序Canvas（Phase 2原生）。

### 11.4 启发3：关系变化影响分析（Phase 2实现）

**UA做法**：代码diff→受影响函数→可视化影响范围。

**EventLink借鉴**：实体属性变化→受影响关联和Todo→生成影响报告。
...
### 11.5 启发4：角色自适应UI（Phase 2实现）

**UA做法**：初级开发者看简化视图，PM看业务流程，资深工程师看完整依赖。

**EventLink借鉴**：不同商务角色看到不同粒度。
...
**技术选型**：PostgreSQL pgvector扩展（Phase 2），无需额外引入向量数据库。

### 11.7 启发6：商业域映射（Phase 2实现）

**UA做法**：domain-analyzer Agent从代码中提取业务域、流程和步骤。

**EventLink借鉴**：从零散事件中识别商业域和业务流程。
...
| 启发 | 价值 | 难度 | 实现阶段 | 依赖 |
|------|------|------|----------|------|
| 会前简报包 | ⭐⭐⭐⭐⭐ | 低 | **Phase 1** | TTSScriptComposer |
| 关系图谱交互 | ⭐⭐⭐⭐ | 中 | Phase 1 Plus | D3.js |
| 关系变化影响分析 | ⭐⭐⭐⭐ | 高 | Phase 2 | 事件历史+关联强度计算 |
| 角色自适应UI | ⭐⭐⭐ | 中 | Phase 2 | 用户角色配置 |
| 向量语义搜索 | ⭐⭐⭐ | 高 | Phase 2 | pgvector扩展 |
| 商业域映射 | ⭐⭐ | 高 | Phase 2 | 大量事件数据+LLM |
