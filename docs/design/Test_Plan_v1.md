...
#### TC-DEPLOY-004: 托管PoC→Phase1迁移数据完整性验证
**目标**: 验证从托管PoC迁移至Phase1环境数据完整性
**前置条件**: 托管PoC环境有生产数据
**测试步骤**:
1. 导出托管PoC全量数据
2. 执行迁移脚本至Phase1环境
...
*④ 新增§21 托管PoC部署验证测试（5个用例: TC-DEPLOY-001 HTTPS访问验证 + TC-DEPLOY-002 微信小程序登录验证 + TC-DEPLOY-003 数据隔离验证 + TC-DEPLOY-004 Phase1迁移数据完整性验证 + TC-DEPLOY-005 备份恢复验证）*
