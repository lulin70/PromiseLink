...
| Week 3 | 端到端集成 + 安全测试 + **F-49日视图** + E2E真实场景 + **回归测试** + **托管PoC部署验证** | E2E场景100%覆盖 |
...
#### TC-W1-001: 标准名片解析
**目标**: 验证OCR+LLM提取准确率
> ⚠️ PoC阶段仅验证OCR+LLM自建链路，不验证外部数字名片API对接
**输入**: 10张标准名片图片（已脱敏）
**期望**:
- 姓名提取准确率 ≥95%
- 公司提取准确率 ≥90%
...
**用户画像**: 新用户首次使用PromiseLink，验证核心4屏流程的流畅度
**完整步骤**:
...
name: PromiseLink Test Pipeline v2.0
on:
  push:
...
```yaml
# .env.test
DATABASE_URL=postgresql://test:test@localhost:5432/promiselink_test
...
```markdown
# PromiseLink Sprint X 测试报告（v2.0模板）
...
**注意**: 第13项（RS-01）为P0阻塞项，即使其他13项全部通过，若RS-01被绕过则不得进入专业版。
...
3. 验证返回值与存储值一致
4. 调用CarryMem.get_system_prompt(context="promiselink_session")
...
**用户画像**: 长期使用PromiseLink的用户，验证跨日连续使用的完整闭环
...
#### TC-E2E-100: 名片小程序扫码→数据传入PromiseLink→Entity创建→关联发现验证

**目标**: 验证名片小程序扫码后数据传入PromiseLink的完整链路
...
| 2 | 用户确认并提交 | 数据通过API传入PromiseLink |
| 3 | PromiseLink创建Event | event_type=card_save, source=iamhere |
...
## 21. 托管PoC部署验证测试 [v4.9新增]

### 21.1 托管PoC部署测试用例

#### TC-DEPLOY-001: 托管PoC HTTPS访问验证
**目标**: 验证托管PoC环境HTTPS访问正常
**前置条件**: 托管PoC环境已部署，SSL证书已配置
**测试步骤**:
1. 通过HTTPS访问托管PoC域名
2. 验证SSL证书有效性
3. 验证HTTP自动重定向至HTTPS
...
- HTTPS访问返回200
- SSL证书有效且未过期
- HTTP请求自动301/302重定向至HTTPS
- API端点HTTPS响应正常

**验收标准**: HTTPS访问正常 + 证书有效 + HTTP重定向 ✅

---

#### TC-DEPLOY-002: 托管PoC微信小程序登录验证
**目标**: 验证微信小程序通过托管PoC环境正常登录
**前置条件**: 微信小程序已配置托管PoC域名，wx.login可用
**测试步骤**:
...
#### TC-DEPLOY-003: 托管PoC数据隔离验证(不同用户)
**目标**: 验证托管PoC环境多用户数据隔离
**前置条件**: 至少2个不同用户已注册
**测试步骤**:
1. 用户A创建Event/Entity/Todo
2. 用户B查询数据
3. 验证用户B无法看到用户A的数据
4. 用户A查询自己的数据，验证完整性
...
#### TC-DEPLOY-004: 托管PoC→专业版迁移数据完整性验证
**目标**: 验证从托管PoC迁移至专业版环境数据完整性
**前置条件**: 托管PoC环境有生产数据
**测试步骤**:
1. 导出托管PoC全量数据
2. 执行迁移脚本至专业版环境
3. 对比迁移前后数据条数
4. 抽样验证关键字段一致性
5. 验证关联关系(Event-Entity-Association-Todo)完整性
...
**验收标准**: 迁移数据100%完整 + 关联关系无断裂 ✅

---

#### TC-DEPLOY-005: 托管PoC备份恢复验证
**目标**: 验证托管PoC环境备份与恢复机制
**前置条件**: 托管PoC环境有生产数据，备份脚本已配置
**测试步骤**:
1. 执行备份脚本，生成备份文件
2. 验证备份文件完整性（文件大小、校验和）
3. 模拟数据丢失场景
4. 执行恢复脚本
5. 验证恢复后数据完整性
...
## 22. [v5.0新增] F-67/F-68/F-69 功能测试 + E2E + 安全测试

### F-67 关系推进卡前端对接测试

#### 单元测试（10个）

##### TC-F67-001: 修正API路径后relationship-brief返回真实数据
**目标**: 验证修正API路径后，relationship-brief接口返回真实数据而非Mock数据
**前置条件**: 后端API已部署，API路径已修正
**测试步骤**:
1. 调用修正后的relationship-brief API路径
2. 验证返回数据非Mock占位数据
3. 验证返回数据包含真实联系人信息
...
**验收标准**: relationship-brief返回真实数据 ✅

---

##### TC-F67-002: concern字段{category,detail}映射正确
**目标**: 验证concern字段从后端到前端的{category,detail}映射正确
**前置条件**: relationship-brief API返回包含concern字段
**测试步骤**:
1. 获取relationship-brief数据
2. 检查concern字段结构
3. 验证每个concern包含category和detail两个子字段
4. 验证category和detail值与后端一致

**期望结果**:
- concern字段为对象数组
- 每个对象包含category和detail
- 映射值与后端源数据一致

**验收标准**: concern字段{category,detail}映射100%正确 ✅

---

##### TC-F67-003: 12模块数据完整展示
**目标**: 验证关系推进卡12个模块数据完整展示
**前置条件**: 联系人有完整的交互数据
**测试步骤**:
1. 打开关系推进卡页面
2. 逐一检查12个模块是否渲染
...
##### TC-F67-004: 关系阶段进度条渲染正确
**目标**: 验证关系阶段进度条正确渲染当前阶段
**前置条件**: 联系人已分配关系阶段
**测试步骤**:
1. 打开关系推进卡
2. 检查进度条渲染
...
##### TC-F67-005: 承诺列表从Brief聚合API获取
**目标**: 验证承诺列表数据从Brief聚合API获取，而非独立调用
**前置条件**: Brief聚合API已包含承诺数据
**测试步骤**:
1. 打开关系推进卡
2. 监控网络请求
3. 验证承诺数据来自Brief聚合API
4. 验证无额外的独立承诺API调用
...
**验收标准**: 承诺列表从Brief聚合API获取 ✅

---

##### TC-F67-006: 推荐回应可复制到剪贴板
**目标**: 验证推荐回应内容可复制到剪贴板
**前置条件**: 关系推进卡显示推荐回应
**测试步骤**:
1. 打开关系推进卡
...
##### TC-F67-007: 无Mock回退（API失败时showToast而非Mock）
**目标**: 验证API失败时不回退到Mock数据，而是显示Toast提示
**前置条件**: API可模拟失败场景
**测试步骤**:
1. 模拟API返回错误
2. 打开关系推进卡
...
**验收标准**: API失败时showToast而非Mock回退 ✅

---

##### TC-F67-008: 空状态处理（无承诺/无关注时友好提示）
**目标**: 验证无承诺或无关注点时显示友好的空状态提示
**前置条件**: 联系人无承诺和关注点数据
**测试步骤**:
1. 打开无承诺数据的联系人推进卡
2. 检查承诺区域空状态
...
##### TC-F67-009: PII字段已解密展示
**目标**: 验证PII字段在前端展示时已正确解密
**前置条件**: 后端PII字段已加密存储
**测试步骤**:
1. 获取关系推进卡数据
2. 检查PII字段（姓名、电话等）
...
##### TC-F67-010: user_id过滤验证
**目标**: 验证关系推进卡数据按user_id正确过滤
**前置条件**: 多用户系统，不同用户有不同联系人数据
**测试步骤**:
1. 用户A访问关系推进卡
2. 验证仅返回用户A的联系人数据
3. 用户B访问关系推进卡
...
### F-68 Promise兑现状态追踪测试

#### 单元测试（12个）

##### TC-F68-001: fulfillment_status枚举值合法性
**目标**: 验证fulfillment_status字段仅接受合法枚举值
**前置条件**: fulfillment_status枚举已定义
**测试步骤**:
1. 设置fulfillment_status为每个合法枚举值
2. 验证保存成功
3. 设置fulfillment_status为非法值
4. 验证保存失败
...
**验收标准**: fulfillment_status枚举值合法性验证通过 ✅

---

##### TC-F68-002: fulfillment_status与Todo.status独立性
**目标**: 验证fulfillment_status与Todo.status字段相互独立
**前置条件**: Todo同时有status和fulfillment_status字段
**测试步骤**:
1. 创建promise类型Todo，status=pending, fulfillment_status=pending
2. 更新status为done，验证fulfillment_status不变
3. 更新fulfillment_status为fulfilled，验证status不变

**期望结果**:
- 修改status不影响fulfillment_status
- 修改fulfillment_status不影响status
- 两个字段完全独立

**验收标准**: fulfillment_status与Todo.status独立性验证通过 ✅

---

##### TC-F68-003: my_promise自动overdue（due_date过期后系统标记）
**目标**: 验证my_promise类型承诺在due_date过期后系统自动标记为overdue
**前置条件**: my_promise类型承诺存在，due_date已设置
**测试步骤**:
1. 创建my_promise，due_date设为过去日期
2. 运行overdue检测任务
3. 验证fulfillment_status自动变为overdue

**期望结果**:
- my_promise到期后自动标记overdue
- overdue_notified_at记录标记时间
- 无需用户手动操作

**验收标准**: my_promise自动overdue验证通过 ✅

---

##### TC-F68-004: their_promise不可自动overdue（仅用户手动）
**目标**: 验证their_promise类型承诺不可被系统自动标记为overdue
**前置条件**: their_promise类型承诺存在，due_date已过期
**测试步骤**:
1. 创建their_promise，due_date设为过去日期
2. 运行overdue检测任务
3. 验证fulfillment_status仍为pending
4. 用户手动标记overdue
5. 验证手动标记成功

**期望结果**:
- 系统不自动标记their_promise为overdue
- 用户可手动标记
- 手动标记后状态正确

**验收标准**: their_promise不可自动overdue验证通过 ✅

---

##### TC-F68-005: 承诺看板my-promises视图过滤
**目标**: 验证承诺看板my-promises视图仅显示my_promise类型
**前置条件**: 存在多种action_type的承诺
**测试步骤**:
1. 创建my_promise和their_promise类型承诺
2. 切换到my-promises视图
3. 验证仅显示my_promise类型

**期望结果**:
- my-promises视图仅包含my_promise
- 不包含their_promise
- 过滤准确率100%

**验收标准**: my-promises视图过滤正确 ✅

---

##### TC-F68-006: 承诺看板their-promises视图过滤
**目标**: 验证承诺看板their-promises视图仅显示their_promise类型
**前置条件**: 存在多种action_type的承诺
**测试步骤**:
1. 创建my_promise和their_promise类型承诺
2. 切换到their-promises视图
3. 验证仅显示their_promise类型

**期望结果**:
- their-promises视图仅包含their_promise
- 不包含my_promise
- 过滤准确率100%

**验收标准**: their-promises视图过滤正确 ✅

---

##### TC-F68-007: PATCH fulfillment状态转换合法性
**目标**: 验证fulfillment_status状态转换遵循合法路径
**前置条件**: fulfillment_status状态机已定义
**测试步骤**:
1. 测试合法转换: pending → fulfilled
2. 测试合法转换: pending → overdue
3. 测试合法转换: overdue → fulfilled
4. 测试非法转换: fulfilled → pending
5. 验证非法转换被拒绝
...
**验收标准**: PATCH fulfillment状态转换合法性验证通过 ✅

---

##### TC-F68-008: 承诺统计API正确计算兑现率
**目标**: 验证承诺统计API正确计算兑现率
**前置条件**: 存在多种状态的承诺
**测试步骤**:
1. 创建10条承诺: 6条fulfilled, 2条overdue, 2条pending
2. 调用承诺统计API
3. 验证兑现率=60%

**期望结果**:
- 兑现率=fulfilled/total×100%
- 统计数值精确
- 分母包含所有状态
...
##### TC-F68-009: fulfilled_at自动记录时间
**目标**: 验证承诺标记为fulfilled时自动记录fulfilled_at时间
**前置条件**: 承诺存在且状态为pending或overdue
**测试步骤**:
1. 将承诺状态更新为fulfilled
2. 检查fulfilled_at字段
3. 验证时间戳为当前时间
...
**验收标准**: fulfilled_at自动记录时间验证通过 ✅

---

##### TC-F68-010: overdue_notified_at记录逾期通知时间
**目标**: 验证逾期通知发送时自动记录overdue_notified_at时间
**前置条件**: 承诺已overdue，通知已发送
**测试步骤**:
1. 承诺自动标记overdue
2. 触发逾期通知
3. 检查overdue_notified_at字段
4. 验证时间戳正确

**期望结果**:
- overdue_notified_at自动填充
- 时间戳与通知发送时间一致
- 仅在通知实际发送时记录

**验收标准**: overdue_notified_at记录逾期通知时间验证通过 ✅

---

##### TC-F68-011: 非promise类型Todo的fulfillment_status无语义
**目标**: 验证非promise类型Todo的fulfillment_status字段无语义
**前置条件**: 存在cooperation_signal/risk等非promise类型Todo
**测试步骤**:
1. 创建非promise类型Todo
2. 检查fulfillment_status字段
3. 验证该字段不影响非promise Todo的行为

**期望结果**:
- 非promise Todo的fulfillment_status为null或默认值
- 不影响Todo的正常状态流转
- 统计API不纳入非promise Todo

**验收标准**: 非promise类型Todo的fulfillment_status无语义验证通过 ✅

---

##### TC-F68-012: user_id隔离验证
**目标**: 验证承诺数据按user_id严格隔离
**前置条件**: 多用户系统，不同用户有不同承诺数据
**测试步骤**:
1. 用户A创建承诺
2. 用户B查询承诺列表
3. 验证用户B看不到用户A的承诺
...
##### TC-F68-I01: 完整承诺闭环（创建→提取→到期→提醒→兑现→统计）
**目标**: 验证承诺从创建到兑现的完整闭环流程
**前置条件**: Pipeline完整运行，通知服务可用
**测试步骤**:
1. 录入包含承诺的会议纪要
2. Pipeline提取承诺Todo
3. 等待due_date到期
...
##### TC-F68-I02: Pipeline step_05输出fulfillment_status
**目标**: 验证Pipeline step_05正确输出fulfillment_status字段
**前置条件**: Pipeline运行至step_05
**测试步骤**:
1. 运行Pipeline处理包含承诺的输入
2. 检查step_05输出
3. 验证fulfillment_status字段存在且为合法枚举值

**期望结果**:
- step_05输出包含fulfillment_status
- 初始值为pending
- 枚举值合法

**验收标准**: Pipeline step_05输出fulfillment_status验证通过 ✅

---

##### TC-F68-I03: step_08通知promise_due_reminder
**目标**: 验证Pipeline step_08正确生成promise_due_reminder通知
**前置条件**: 承诺即将到期
**测试步骤**:
1. 创建即将到期的承诺
2. 运行step_08通知生成
3. 验证生成promise_due_reminder类型通知
4. 验证通知内容包含承诺详情

**期望结果**:
- step_08生成promise_due_reminder通知
- 通知内容正确
- 通知时机合理

**验收标准**: step_08通知promise_due_reminder验证通过 ✅

---

##### TC-F68-I04: step_12 Brief聚合兑现状态
**目标**: 验证Pipeline step_12 Brief聚合API包含兑现状态
**前置条件**: Pipeline运行至step_12
**测试步骤**:
1. 运行Pipeline至step_12
2. 查询Brief聚合API
3. 验证承诺数据包含fulfillment_status
4. 验证兑现状态正确聚合
...
**验收标准**: step_12 Brief聚合兑现状态验证通过 ✅

---

##### TC-F68-I05: my_promise overdue自动触发通知
**目标**: 验证my_promise自动overdue后触发通知
**前置条件**: my_promise到期，overdue自动标记已执行
**测试步骤**:
1. my_promise到期被自动标记overdue
2. 验证overdue通知自动触发
3. 验证通知内容包含承诺详情
...
**验收标准**: my_promise overdue自动触发通知验证通过 ✅

---

##### TC-F68-I06: their_promise overdue需用户确认
**目标**: 验证their_promise overdue需用户手动确认，不自动触发通知
**前置条件**: their_promise到期
**测试步骤**:
1. their_promise到期
2. 验证系统不自动标记overdue
3. 验证不自动触发通知
...
**期望结果**:
- 系统不自动标记their_promise overdue
- 不自动触发通知
- 用户确认后正常触发

**验收标准**: their_promise overdue需用户确认验证通过 ✅

---

##### TC-F68-I07: 承诺看板分页和排序
**目标**: 验证承诺看板分页和排序功能正常
**前置条件**: 存在大量承诺数据
**测试步骤**:
1. 创建超过20条承诺
...
##### TC-F68-I08: 兑现后Brief自动更新
**目标**: 验证承诺兑现后Brief聚合数据自动更新
**前置条件**: Brief数据已缓存
**测试步骤**:
1. 查看Brief中承诺兑现统计
2. 标记一条承诺为fulfilled
...
##### TC-F69-001: 提醒时机算法计算正确
**目标**: 验证提醒时机算法根据关系阶段和交互频率正确计算
**前置条件**: 联系人有交互历史和关系阶段
**测试步骤**:
1. 设置不同关系阶段的联系人
2. 运行提醒时机算法
...
##### TC-F69-004: 每日提醒清单按优先级排序
**目标**: 验证每日提醒清单按优先级从高到低排序
**前置条件**: 存在多个不同优先级的提醒
**测试步骤**:
1. 生成多个不同优先级的提醒
2. 查看每日提醒清单
...
2. 验证提醒状态变为completed
3. 验证关联Todo状态更新
4. 验证reminder_logs记录action_taken
...
##### TC-F69-006: 推迟操作+自动重新计算
**目标**: 验证推迟操作后系统自动重新计算提醒时机
**前置条件**: 提醒已生成
**测试步骤**:
1. 点击推迟按钮
2. 验证提醒状态变为postponed
...
##### TC-F69-008: 提醒偏好设置CRUD
**目标**: 验证提醒偏好设置的增删改查功能
**前置条件**: 用户已登录
**测试步骤**:
1. 创建提醒偏好设置（静默时段、每日上限等）
2. 读取偏好设置
3. 更新偏好设置
4. 删除偏好设置
...
##### TC-F69-009: reminder_logs记录action_taken
**目标**: 验证reminder_logs正确记录用户操作
**前置条件**: 提醒已生成
**测试步骤**:
1. 对提醒执行完成操作
2. 检查reminder_logs
3. 验证action_taken字段为"completed"
4. 对另一提醒执行推迟操作
5. 验证action_taken字段为"postponed"
...
**验收标准**: reminder_logs记录action_taken验证通过 ✅

---

##### TC-F69-010: user_id隔离验证
**目标**: 验证提醒数据按user_id严格隔离
**前置条件**: 多用户系统
**测试步骤**:
1. 用户A生成提醒
2. 用户B查询提醒清单
...
##### TC-F69-I01: 承诺到期→自动生成提醒→用户操作→日志记录
**目标**: 验证承诺到期到提醒操作的完整流程
**前置条件**: 承诺即将到期
**测试步骤**:
1. 承诺到期触发提醒生成
...
**验收标准**: 承诺到期→提醒→操作→日志全链路通过 ✅

---

##### TC-F69-I02: 连续5天触发→第6天疲劳控制
**目标**: 验证连续5天触发提醒后，第6天疲劳度控制生效
**前置条件**: 用户连续5天收到提醒
**测试步骤**:
1. 模拟连续5天每天触发提醒
2. 第6天生成新提醒
...
##### TC-F69-I03: 用户调整偏好→新偏好生效
**目标**: 验证用户调整提醒偏好后新偏好立即生效
**前置条件**: 用户有默认偏好设置
**测试步骤**:
1. 用户将每日上限从5改为3
2. 生成新提醒
...
**验收标准**: 用户调整偏好→新偏好生效验证通过 ✅

---

##### TC-F69-I04: 推迟后重新计算时机
**目标**: 验证推迟提醒后系统重新计算下次提醒时机
**前置条件**: 提醒已生成
**测试步骤**:
1. 用户推迟一条提醒
2. 系统重新计算提醒时机
...
##### TC-F69-I05: 4种提醒类型分别生成
**目标**: 验证4种提醒类型（承诺到期、关系冷却、关注点跟进、帮助提议）分别正确生成
**前置条件**: 存在触发4种提醒的数据
**测试步骤**:
1. 创建即将到期的承诺→验证承诺到期提醒
2. 创建长时间未联系的联系人→验证关系冷却提醒
3. 创建未确认的关注点→验证关注点跟进提醒
4. 创建需要帮助的场景→验证帮助提议提醒
...
##### TC-F69-I06: APScheduler定时任务执行
**目标**: 验证APScheduler定时任务正确执行提醒生成
**前置条件**: APScheduler已配置定时任务
**测试步骤**:
1. 配置定时任务执行时间
2. 等待定时任务触发
3. 验证提醒生成任务执行
...
#### TC-SEC-001: their_promise overdue不可AI自动标记
**目标**: 验证their_promise类型的overdue状态不可被AI自动标记，防止误判对方违约
**前置条件**: their_promise承诺已到期
**测试步骤**:
1. 创建their_promise类型承诺
2. due_date过期后运行AI处理
3. 验证AI不自动标记overdue
4. 验证仅用户可手动标记

**期望结果**:
- AI不自动标记their_promise为overdue
- 仅用户手动操作可标记
- 防止误判对方违约

**验收标准**: their_promise overdue不可AI自动标记 ✅

---

#### TC-SEC-002: 催促话术不含索取性内容
**目标**: 验证催促提醒的话术不包含索取性或不当内容
**前置条件**: 生成催促提醒话术
**测试步骤**:
1. 触发催促提醒生成
2. 检查生成的话术内容
...
#### TC-SEC-003: 提醒数据user_id隔离
**目标**: 验证提醒数据按user_id严格隔离，防止跨用户数据泄露
**前置条件**: 多用户系统
**测试步骤**:
1. 用户A生成提醒数据
2. 用户B尝试访问用户A的提醒
...
**验收标准**: 提醒数据user_id隔离验证通过 ✅

---

#### TC-SEC-004: 关系推进卡PII字段已加密
**目标**: 验证关系推进卡中PII字段在存储层已加密
**前置条件**: 推进卡包含PII字段（姓名、电话等）
**测试步骤**:
1. 直接查询数据库
2. 验证PII字段为密文
3. 通过API获取数据
...
*v4.9更新于2026-06-09，主要变更：*
*① 版本号v4.8→v4.9（对齐PRD v4.9）*
*② §1.2测试范围新增"托管PoC部署验证"测试项*
*③ TC-W1-001/002新增备注：PoC阶段仅验证OCR+LLM自建链路，不验证外部数字名片API对接*
*④ 新增§21 托管PoC部署验证测试（5个用例: TC-DEPLOY-001 HTTPS访问验证 + TC-DEPLOY-002 微信小程序登录验证 + TC-DEPLOY-003 数据隔离验证 + TC-DEPLOY-004 专业版迁移数据完整性验证 + TC-DEPLOY-005 备份恢复验证）*

*v5.0更新于2026-06-11，主要变更：*
*① 版本号v4.9→v5.0*
*② 新增§22 F-67/F-68/F-69功能测试 + E2E + 安全测试（53个用例）*
*③ F-67 关系推进卡前端对接测试 — 单元测试10个（TC-F67-001~010: API路径/concern映射/12模块/进度条/Brief聚合/复制/Mock回退/空状态/PII解密/user_id过滤）*
*④ F-68 Promise兑现状态追踪测试 — 单元测试12个（TC-F68-001~012: 枚举合法性/独立性/自动overdue/手动overdue/看板过滤/状态转换/兑现率/fulfilled_at/overdue_notified_at/非promise语义/user_id隔离）+ 集成测试8个（TC-F68-I01~I08: 闭环/Pipeline step_05/step_08/step_12/自动触发通知/用户确认/分页排序/Brief更新）*
*⑤ F-69 智能跟进提醒测试 — 单元测试10个（TC-F69-001~010: 时机算法/疲劳度/静默时段/优先级排序/一键完成/推迟/忽略/偏好CRUD/日志/user_id隔离）+ 集成测试6个（TC-F69-I01~I06: 闭环/疲劳控制/偏好生效/推迟重算/4种类型/APScheduler）*
*⑥ E2E用户旅程测试3个（TC-E2E-001 承诺闭环旅程 + TC-E2E-002 关系推进卡旅程 + TC-E2E-003 每日提醒旅程）*
*⑦ 安全测试4个（TC-SEC-001 their_promise不可AI自动标记 + TC-SEC-002 催促话术无索取性内容 + TC-SEC-003 提醒数据user_id隔离 + TC-SEC-004 PII字段加密）*

*v5.1更新于2026-06-11，主要变更：*
*① 版本号v5.0→v5.1（对齐三层产品模型：基础版/专业版/定制版）*
*② Phase 1→专业版、Phase 2→定制版术语替换*
*③ 新增§23 三层产品模型E2E测试（5个用例: TC-TIER-001 基础版Taro H5 E2E + TC-TIER-002 专业版网关中继E2E + TC-TIER-003 基础版→专业版升级 + TC-TIER-004 网关故障降级 + TC-TIER-005 Docker一键安装）*
...
2. 使用PoC登录方式完成身份认证
3. 创建名片Event，验证Entity抽取
4. 创建会议Event，验证Todo生成
5. 查看关系推进卡，验证12模块数据展示
6. 验证无网关中继连接（基础版不依赖外部网关）
7. 验证AI功能不可用（基础版无AI代理）

**期望结果**:
- Taro H5页面正常加载，无白屏或JS错误
- 核心CRUD操作（Event/Entity/Todo）正常
- 关系推进卡数据完整展示
- 无网关WebSocket连接尝试
- AI相关功能入口隐藏或提示"升级至专业版"

**验收标准**: 基础版Taro H5核心流程全部通过 ✅

---

### TC-TIER-002: 专业版网关中继E2E测试（WebSocket连接+请求转发+AI代理+计数限流）
...
**测试步骤**:
1. 客户端通过 `ws://gateway/ws/relay` 建立WebSocket连接
2. 调用 `POST /api/v1/relay/register` 注册用户PC连接
3. 调用 `GET /api/v1/relay/status` 确认连接状态为 `connected`
4. 通过网关中继发送业务请求，验证请求正确转发至本地服务
5. 调用 `POST /api/v1/relay/ai-proxy` 发起AI代理请求
6. 验证AI代理响应正确返回
7. 调用 `GET /api/v1/relay/ai-quota` 查询AI配额
8. 连续发起AI请求至配额上限，验证计数限流生效
9. 超出配额后验证请求被拒绝（HTTP 429）
...
**验收标准**: 专业版网关中继全链路E2E通过 ✅

---

### TC-TIER-003: 基础版→专业版升级测试（同一SQLite，加网关连接）
...
6. 建立网关WebSocket连接
7. 调用 `POST /api/v1/relay/register` 注册连接
8. 验证AI代理功能可用
9. 验证AI配额初始值正确
...
**验收标准**: 基础版→专业版升级数据零丢失 + 网关功能正常 ✅

---

### TC-TIER-004: 网关故障降级测试（专业版→基础版自动降级）
...
**测试步骤**:
1. 专业版正常运行，确认网关连接状态为 `connected`
2. 模拟网关服务宕机（停止网关容器或断开网络）
3. 验证WebSocket连接断开检测（超时或错误回调）
4. 验证系统自动降级至基础版模式
5. 在降级模式下执行核心CRUD操作（创建Event/查看Todo）
6. 验证AI代理功能不可用，返回友好提示
7. 恢复网关服务
8. 验证系统自动重连网关
9. 验证AI代理功能恢复可用
10. 验证降级期间的数据无丢失
...
- 网关断开后3秒内检测到故障
- 自动降级至基础版，核心CRUD功能正常
- AI功能降级提示"网关连接中断，AI功能暂时不可用"
- 网关恢复后自动重连，AI功能恢复
- 降级期间产生的数据完整无丢失

**验收标准**: 网关故障自动降级 + 恢复后自动升级 + 数据零丢失 ✅
