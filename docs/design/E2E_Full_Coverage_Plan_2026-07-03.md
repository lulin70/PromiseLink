...
**动机**: 用户硬约束"测试不充分对系统的危害对用户的危害"——后端 API 必须有前端 UI 操作触发，否则为"幽灵功能"；小程序 Pro 专属功能必须在激活态下 E2E 全覆盖
...
2. **小程序 Pro 激活态全覆盖**：语音输入、OCR 名片扫描、邮件同步、CSV 导入全部 Pro 专属功能在激活态下端到端测试
...
- [ ] Pro 全部 P0 专属功能（语音/OCR/CSV/邮件/微信转发）端到端测试通过
