...
HTTP→WSS 业务请求中继：小程序通过 HTTP 调用网关，网关通过 WSS 长连接转发到用户本地基础版桌面应用，返回响应。这是小程序访问用户本地数据的核心通道。
...
> **架构说明**: 此端点是三层架构 L1→L2→L3 的核心：L1 小程序 HTTP → L2 网关 `/relay/request` → L2 网关通过 WSS 转发 → L3 用户本地基础版桌面应用。用户电脑必须保持开机且基础版服务运行中。
