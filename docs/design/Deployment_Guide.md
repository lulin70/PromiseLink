...
> **版本**: 0.4.8（POC阶段 — Insight Engine + 依赖性/场景匹配 + 语义搜索 + EmailAdapter/CSV导入 + 托管PoC部署 部署）
> **日期**: 2026-06-08
...
| **托管PoC** | 云端轻量服务器 + Docker | SQLite | 无 | ~50元/月 | 单用户（微信小程序接入） | 非技术用户可用、小程序可访问 |
...
| 项目 | PoC | 托管PoC | Phase1 | Phase2 |
|------|-----|---------|--------|--------|
| **操作系统** | macOS / Linux / Windows(WSL2) | Ubuntu 22.04+ | Ubuntu 22.04+ / CentOS 8+ | K8s 兼容 OS |
| **Docker** | Docker Desktop 24+ | Docker Engine 24+ | Docker Engine 24+ | containerd |
| **Docker Compose** | v2.20+ | v2.20+ | v2.20+ | N/A（Helm） |
| **Python** | 3.11+（宿主机调试用） | N/A（容器内） | N/A（容器内） | N/A |
| **内存** | ≥4GB 可用 | ≥4GB 可用 | ≥4GB 可用 | 按Pod配置 |
| **磁盘** | ≥2GB 可用 | ≥20GB 可用 | ≥20GB 可用 | PVC 按需 |
| **CPU** | ≥2核 | ≥2核 | ≥2核 | 按Pod配置 |
| **网络** | 需访问LLM API | 需公网IP+域名 | 需公网IP+域名 | 集群内网络 |
| **LLM API Key** | 必需 | 必需 | 必需 | 必需 |
...
| 依赖 | 版本要求 | 安装验证 | 说明 |
|------|----------|----------|------|
| **Python: edge-tts** | ≥6.1.0 | `pip show edge-tts` | Azure Edge-TTS（免费在线TTS），Phase1语音合成核心 |
...
RUN mkdir -p /var/cache/eventlink/tts && chown eventlink:eventlink /var/cache/eventlink/tts
```

| 配置项 | 值 | 说明 |
|--------|-----|------|
| Python依赖 | `edge-tts>=6.1.0` | Azure Edge-TTS，免费在线TTS |
| TTS缓存目录 | `/var/cache/eventlink/tts` | 已合成音频持久化缓存 |
| 目录权限 | `eventlink:eventlink` | 非root用户所有，安全最佳实践 |
...
Voice功能不需要新增服务（复用现有 `eventlink-api` 服务），但需在现有服务配置中追加以下内容：
...
volumes:
  - ./data:/data
  - ./data/tts_cache:/var/cache/eventlink/tts    # [F-50新增]
```
...
[0.4.8新增] 本节面向**不具备服务器运维能力的非技术用户**，提供一种介于本地PoC和Phase1之间的轻量云端部署方案。用户只需购买一台轻量云服务器，即可通过微信小程序访问EventLink，无需自行管理本地Docker环境。

#### 2.9.1 概述

**适用场景**：
- 用户无法在本地运行Docker Desktop（如公司电脑受限、无Linux/macOS环境）
- 需要微信小程序随时访问EventLink（本地PoC无法被小程序访问）
- 不想投入Phase1级别的运维成本（PG+Redis+Nginx全套）
- 希望以最低成本验证EventLink核心流程
...
│  │                                                           │  │
│  │  ┌─────────────────┐    ┌─────────────────────────────┐  │  │
│  │  │  eventlink-api   │    │  Nginx（反向代理+SSL终止）   │  │  │
│  │  │  :8000           │◄───│  :443 → :8000               │  │  │
...
│  └───────────────────────────────────────────────────────────┘  │
│                                                                 │
│  数据持久化：./data/eventlink_poc.db（Docker Volume）            │
└─────────────────────────────────────────────────────────────────┘
          ▲
          │ HTTPS
          │
...
**架构要点**：
- 服务器上仅运行2个Docker容器：eventlink-api + Nginx
...
#    例如：api.eventlink.com → 123.45.67.89

# 2. 安装Nginx和Certbot
sudo apt update && sudo apt install -y nginx certbot python3-certbot-nginx

# 3. 配置Nginx基础反向代理
sudo tee /etc/nginx/sites-available/eventlink <<'EOF'
server {
    listen 80;
...
sudo ln -sf /etc/nginx/sites-available/eventlink /etc/nginx/sites-enabled/
sudo nginx -t && sudo systemctl reload nginx

# 4. 获取Let's Encrypt SSL证书（自动配置HTTPS）
sudo certbot --nginx -d api.eventlink.com
...
nano .env.poc
# 必填项：
#   EVENTLINK_SECRET_KEY=<用下方命令生成>
#   LLM_API_KEY=<你的API Key>
#   LLM_BASE_URL=<对应的API地址>
#   EVENTLINK_CORS_ORIGINS=["https://api.eventlink.com"]
...
# 外部HTTPS验证
curl https://api.eventlink.com/api/v1/health
```
...
| 配置项 | 域名 |
|--------|------|
| request合法域名 | `https://api.eventlink.com` |
| uploadFile合法域名 | `https://api.eventlink.com` |
| downloadFile合法域名 | `https://api.eventlink.com` |
...
| 备份项 | 方式 | 频率 | 保留周期 |
|--------|------|------|----------|
| SQLite数据库 | `cp data/eventlink_poc.db data/backup/` | 每日 | 7天 |
| .env配置 | 手动备份到安全位置 | 变更时 | 永久 |
| Nginx配置 | `/etc/nginx/sites-available/eventlink` | 变更时 | 永久 |

**自动备份脚本**（crontab）：

```bash
# 每天凌晨3点备份SQLite数据库
0 3 * * * cd /path/to/EventLink && mkdir -p data/backup && cp data/eventlink_poc.db data/backup/eventlink_poc_$(date +\%Y\%m\%d).db && find data/backup -name "eventlink_poc_*.db" -mtime +7 -delete
```
...
当业务增长需要升级到Phase1（PostgreSQL + Redis + 完整生产配置）时，按以下步骤迁移：

```
托管PoC（SQLite） ──数据导出──▶ Phase1（PostgreSQL）
```
...
#   DATABASE_URL=postgresql+asyncpg://eventlink:STRONG_PASSWORD@postgres:5432/eventlink
#   REDIS_ENABLED=true
#   REDIS_URL=redis://redis:6379/0

# 5. 启动Phase1服务（含PG+Redis）
...
> **迁移要点**：托管PoC到Phase1的迁移与§7.5 SQLite→PostgreSQL升级路径完全一致，核心是数据导出→方言转换→PG导入。
...
> **成本对比**：托管PoC（~54元/月）vs Phase1（~200元/月），托管PoC成本约为Phase1的1/4，适合预算有限的早期验证阶段。
...
```bash
# 1. 验证edge-tts可用性
docker compose exec eventlink-api python -c "
...
# 2. 检查TTS缓存目录
docker compose exec eventlink-api ls -la /var/cache/eventlink/tts/

# 3. 查看Voice相关日志
docker compose logs eventlink-api 2>&1 | grep -i "voice\|tts\|asr\|nlu"

# 4. 检查Voice环境变量
docker compose exec eventlink-api env | grep -E "^TTS_|^VOICE_"
```
...
| `ASR_AUDIO_MAX_SIZE` | `10MB` | PoC足够，Phase1按需调整 | ASR上传音频最大尺寸限制 |
...
- [ ] **TTS缓存目录权限**：`/var/cache/eventlink/tts` 归属 `eventlink` 用户（非world-writable），验证：`stat /var/cache/eventlink/tts`
...
| **语音助手(F-50)** | | | | |
| `VOICE_ENABLED` | `true` | 否 | Phase1+ | Feature Flag：是否启用语音功能（`true`/`false`） |
| `TTS_VOICE_NAME` | `zh-CN-XiaoxiaoNeural` | 否 | Phase1+ | Edge-TTS语音角色名称 |
| `TTS_RATE` | `-10%` | 否 | Phase1+ | TTS语速调节（`-50%~+100%`） |
| `TTS_CACHE_TTL` | `86400` | 否 | Phase1+ | TTS缓存过期时间（秒），默认24h |
| `NLU_RULE_THRESHOLD` | `0.85` | 否 | Phase1+ | NLU规则引擎意图识别阈值 |
| `VOICE_SESSION_TTL` | `604800` | 否 | Phase1+ | 语音会话保留时间（秒），默认7d |
...
| **0.4.8** | **2026-06-08** | **托管PoC部署模式：①§1.1 三阶段部署策略表新增"托管PoC"行(云端轻量服务器+SQLite+~50元/月+小程序接入) ②§1.2 环境要求表新增"托管PoC"列 ③§2.9 托管PoC部署模式[0.4.8新增] — 2.9.1 概述(适用场景+与本地PoC对比表) 2.9.2 架构图(云端服务器+Docker Compose+eventlink-api+Nginx+SQLite+微信小程序+LLM API) 2.9.3 前置条件(云服务器2C4G/域名/SSL证书/LLM API Key/SSH+域名备案说明) 2.9.4 部署步骤7步(购买服务器→域名HTTPS→安装Docker→克隆配置→启动服务→健康检查→小程序白名单) 2.9.5 运维管理(备份策略+自动备份脚本+监控+更新升级) 2.9.6 迁移到Phase1(SQLite导出→PG导入→切换compose文件) 2.9.7 费用估算(~54元/月不含LLM，约为Phase1的1/4)** | **DevSquad** |

---

> **文档状态**: ✅ 0.4.8 POC阶段版本完成（Insight Engine + 依赖性/场景匹配 + 语义搜索 + EmailAdapter/CSV导入 + 托管PoC部署 部署）
