...
#    例如：gateway.promiselink.cn → 123.45.67.89
...
sudo certbot --nginx -d gateway.promiselink.cn
...
#   PROMISELINK_CORS_ORIGINS=["https://gateway.promiselink.cn"]
...
# 外部HTTPS验证
curl https://gateway.promiselink.cn/api/v1/health
```
...
| 配置项 | 域名 |
|--------|------|
| request合法域名 | `https://gateway.promiselink.cn` |
| uploadFile合法域名 | `https://gateway.promiselink.cn` |
| downloadFile合法域名 | `https://gateway.promiselink.cn` |
...
docker run -d \
  ...
  -e LLM_API_KEY=sk-xxx \
  -e POC_SECRET=your-secret \
  -e RELAY_GATEWAY_URL=wss://gateway.promiselink.cn/relay \
...
| 环境变量 | 基础版 | 专业版 | 说明 |
|----------|--------|--------|------|
| `RELAY_GATEWAY_URL` | 不设置 | `wss://gateway.promiselink.cn/relay` | 设置即启用网关中继 |
...
#   RELAY_GATEWAY_URL=wss://gateway.promiselink.cn/relay
...
# 预期输出：relay_client started, connecting to wss://gateway.promiselink.cn/relay
...
| 配置项 | 域名 | 说明 |
|--------|------|------|
| request合法域名 | `https://gateway.promiselink.cn` | API请求（经网关中继） |
| socket合法域名 | `wss://gateway.promiselink.cn` | WebSocket中继连接 |
| uploadFile合法域名 | `https://gateway.promiselink.cn` | 文件上传 |
| downloadFile合法域名 | `https://gateway.promiselink.cn` | 文件下载 |
...
# 3. 功能验证
curl -H "Authorization: Bearer $TOKEN" https://gateway.promiselink.cn/api/v1/entities
curl -H "Authorization: Bearer $TOKEN" https://gateway.promiselink.cn/api/v1/events
```
...
| 小程序请求失败 | 域名未配置白名单 | 检查微信公众平台配置 | 添加 `https://gateway.promiselink.cn` |
