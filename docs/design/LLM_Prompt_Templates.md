...
> **最后更新**: 2026-06-07
> **模板总数**: 25个 (模板0-14 + 模板15-21 + 模板22-23 + 模板24 + [0.4.1新增]模板25共1个)
> **阶段**: POC (0.3.x series)
...
## 25. 模板25：需求提取（Demand Extraction, F-36） [0.4.1新增]

**用途**: 从用户一句话文本中提取需求标签和详情，用于F-36需求录入功能。LLM提取失败时降级为关键词匹配。

**输入变量**:

| 变量 | 类型 | 说明 |
|------|------|------|
| text | string | 用户输入的一句话需求文本 |

**Prompt**:
```
你是一个需求信息提取助手。从用户的一句话中提取需求标签和需求详情。
...
**降级策略**: LLM不可用时，使用关键词正则匹配（`_fallback_extract`）:
...
| 维度 | 模板8 | 模板25 |
|------|-------|--------|
| 输入 | 长文本+指定person_name | 一句话 |
| 输出 | 多条需求+分类+紧迫性+匹配潜力 | 单条tag+detail+person_name |
| 用途 | Pipeline中从Event提取需求 | 用户主动录入需求 |
| 降级 | 无 | 关键词正则匹配 |
...
| 0.4.1 | 2026-06-07 | [F-36新增]模板25(需求提取Demand Extraction,一句话输入→tag+detail+person_name,LLM提取+关键词正则降级,与模板8区别说明);模板总数24→25 |
