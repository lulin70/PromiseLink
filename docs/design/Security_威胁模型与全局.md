# PromiseLink 安全设计文档 — 威胁模型与全局

> **版本**: v3.0 (POC阶段 — 托管PoC部署模式安全更新)
> **拆分日期**: 2026-06-08
...
| **T**ampering（篡改） | API请求参数被中间人篡改 | HTTPS (TLS 1.3) + HMAC请求签名 | ⚠️ 本地PoC阶段HTTP，托管PoC阶段HTTPS(Let's Encrypt TLS 1.2+)，Phase1升级TLS 1.3 |
...
| 攻击面 | 入口点 | 暴露数据 | 威胁 | 防御 | 防御优先级 |
|--------|--------|----------|------|------|------------|
| REST API | `/api/v1/*` | 全部业务数据 | 注入攻击、认证绕过 | JWT认证+输入验证+限流 | P0 |
| 公网API端点(托管PoC) | `https://域名/api/v1/*` | 全部业务数据 | DDoS攻击、暴力破解poc_secret、未授权访问 | HTTPS强制、Rate Limiting、poc_secret认证、可选IP白名单 | P0 |
| LLM Prompt | 事件文本输入 | 用户PII | Prompt注入、数据投毒 | PII脱敏+Prompt安全模板 | P0 |
| 小程序WebView | H5页面URL | 认证凭据 | Token截获、XSS | 临时授权码模式+输入消毒 | P0 |
| 数据库 | SQLite/PG文件 | 加密后PII | SQL注入、直接访问 | 参数化查询+字段加密 | P1 |
| Redis | 6379端口 | Ticket/会话 | 未授权访问 | 绑定内网+认证密码 | P1 |
| Docker | 容器端口映射 | 服务配置 | 容器逃逸 | 最小权限+只读文件系统 | P2 |
...
| 安全措施 | PoC（本地Docker+SQLite） | 托管PoC（云端轻量服务器+SQLite+HTTPS） | Phase1（云端Docker Compose+PG+Redis） | Phase2（生产增强） |
|----------|--------------------------|---------------------------------------|---------------------------------------|-------------------|
| **传输加密** | HTTP（本地内网） | HTTPS（Let's Encrypt TLS 1.2+） | TLS 1.3（Let's Encrypt） | TLS 1.3（商业证书+OCSP） |
| **认证算法** | HS256（简化） | HS256（简化） | RS256（非对称） | RS256 + 设备绑定 |
| **Token传递** | URL参数（开发便利） | URL参数+Ticket模式 | Ticket模式（安全） | Ticket + Cookie HttpOnly |
| **PII加密** | 不加密（开发便利） | AES-256-GCM字段级加密 | AES-256-GCM字段级加密 | AES-256-GCM + TDE |
| **数据库加密** | SQLite明文 | SQLite明文 | pgcrypto字段加密 | TDE透明加密 |
...
| **CORS** | localhost | 微信小程序域名+托管域名白名单 | 生产域名白名单 | 多域名白名单 |
...
| v3.0 | 2026-06-09 | **托管PoC部署模式安全更新**：①§1.2 STRIDE威胁模型Tampering行更新(区分本地PoC/托管PoC/Phase1传输加密状态) ②§1.3 攻击面清单新增"公网API端点(托管PoC)"条目(DDoS/暴力破解poc_secret/未授权访问+HTTPS强制/Rate Limiting/poc_secret认证/可选IP白名单) ③§9.1 各阶段安全措施对比表新增"托管PoC"列(HTTPS+HS256+InMemorySlidingWindow+AES-256-GCM+文件日志+微信+托管CORS) | 架构师 + 安全工程师 |

---

> **文档状态**: ✅ v3.0 更新完成（托管PoC部署模式安全更新：STRIDE Tampering行 + 攻击面清单 + 安全措施对比表）
> **下次审查**: Phase1开发启动前
> **安全负责人**: 架构师
