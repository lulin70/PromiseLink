...
RUN apt-get update && apt-get install -y \
    ...
    curl \
    && rm -rf /var/lib/apt/lists/* \
    && groupadd -r eventlink \
    && useradd -r -g eventlink -d /app -s /sbin/nologin eventlink
...
# Copy application code (src/eventlink/ → /app/src/eventlink/)
COPY src/ ./src/
...
# Run from src/ so Python can find eventlink package
WORKDIR /app/src
CMD ["uvicorn", "eventlink.main:app", "--host", "0.0.0.0", "--port", "8000"]
