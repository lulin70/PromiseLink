...
# 场景2: NLU语音问询 — 覆盖许总描述的7类典型问询
VOICE_QUERIES = [
    ("日程查询", "我今天的会议是什么？", "schedule_query"),
    ("日程查询", "明天有什么安排？", "schedule_query"),
    ("承诺追踪", "我答应老王什么事还没做？", "promise_tracker"),
    ("关系推进", "张总那边到哪一步了？", "relationship_status"),
    ("行动建议", "下一步我该怎么推进？", "action_suggestion"),
...
async def demo_pipeline() -> dict:
    ...
    # ── 开场白 ──
    print(f"  {BOLD}许总的话{RESET}: \"{DIM}我知道老王能搞定装修，但三个月没联系了{RESET}\"")
    ...
    # 1. 初始化数据库
    sub_header("Step 0: 系统初始化")
    await init_db()
    ok("数据库就绪 (SQLite + Alembic migrations)")

    # 2. 创建测试事件
    sub_header("Step 1: 创建事件")
    event_id = str(uuid4())
    ...
    # 3. 运行完整Pipeline
    sub_header("Step 2: 运行11步Pipeline (真实Moka AI)")
    ...
    sub_header("Step 3: 实体提取 (LLM NER)")
    if result.entities:
        for i, entity in enumerate(result.entities):
            print(f"    [{i+1}] {BOLD}{entity.name}{RESET} ({entity.entity_type}) | 置信度={entity.confidence:.0%}")
        ok(f"提取到 {len(result.entities)} 个实体")
    else:
        fail("未提取到实体")

    # Step 4+5: Todo + Promise
    sub_header("Step 4+5: 待办生成 + 承诺分析 (F-45+F-46)")
    if result.todos:
        action_labels = {
            "my_promise": "我的承诺",
            "their_promise": "对方承诺",
    ...
    # Step 8: Briefs
    sub_header("Step 8: 关系推进卡更新 (F-47+F-48)")
    brief_count = 0
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if briefs:
                for brief in briefs:
                    ...
                    score = (brief.brief_data or {}).get("strength_score", 0)
                    modules = len(brief.brief_data or {})
                    print(f"    推进卡: person={brief.person_entity_id[:8] if brief.person_entity_id else '?'}...")
...
async def demo_brief() -> dict:
    ...
    header("场景3: 关系推进卡 — \"张总到哪步了？\"")

    print(f"  {BOLD}许总的问题{RESET}: \"{DIM}张总到哪步了？{RESET}\"")
    print(f"  {BOLD}EventLink的答案{RESET}: 12模块关系画像 + AI驱动的下一步建议\n")
...
async def main() -> None:
    ...
    print(f"  {DIM}演示时间: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}{RESET}")
