...
- **修复**：三语 README 的「文档版本」行与 PRD 链接统一为 `v5.9`（共 6 处）。**里程碑段里的 `PRD v5.2` / `技术设计 v3.2` 一律不动** —— 那是"当时交付所依据的版本"，是历史事实，回填反而丢失可追溯性。
- **技术设计头部 `> 对应PRD: v5.2` 数字不改**，改为在其下补一段「口径说明（2026-09-21 补）」：说明该值指**写作基线**、不随 PRD 升版改写，且本文档对 v5.9 的覆盖是**部分**的（v5.3~v5.9 以附录式增补落在各章节，差异以 PRD 为准），并点明 README 的版本行是"当前 PRD + 本文档版本"的混合口径，与它不是一个口径。
- **可重复门禁（新增）**：`scripts/quality/check_doc_versions.py` —— 抽取 README 三语里的版本**声称**（版本行 + 指向真实文档的链接），与文档头部**声明**的版本比对；已被 `test` job 的 `Doc version consistency gate (③)` 步骤调用，本地同一命令可复跑（当前输出：12 条声称全部一致）。
- **门禁自身的反向探针（内置在测试里，不是一次性手工）**：`tests/test_doc_version_gate.py` 4 条 —— 真实仓库必须绿；文档升到 v9.9 而 README 未跟必须红；README 声称被改旧必须红；**README 选择器失效（一条声称都抽不到）必须红**（禁止"检查 0 条然后绿"）。前三条即 §2.3 要求的反向探针。
