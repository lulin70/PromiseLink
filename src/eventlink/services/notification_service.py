"""Notification service for EventLink.
...
logger = get_logger("eventlink.notification")
...
class NotificationService:
    ...
    async def _send_wechat_template(self, message: NotificationMessage) -> bool:
        ...
        if not self.wechat_app_id or not self.wechat_app_secret:
            logger.warning("wechat_not_configured", action="template_message")
            return False
    ...
    async def _send_wechat_subscribe(self, message: NotificationMessage) -> bool:
        ...
        """
        if not self.wechat_app_id or not self.wechat_app_secret:
            logger.warning("wechat_not_configured", action="subscribe_message")
            return False
