...
async def list_entities(
    ...
    if event_type:
        query = query.where(Entity.entity_type == event_type)
    if status:
    ...
    if event_type:
        count_query = count_query.where(Entity.entity_type == event_type)
    if status:
...
async def delete_entity(
    ...
    await session.execute(
        Todo.__table__.delete().where(Todo.related_entity_id == str(entity_id))
    )
...
async def get_entity_history(
    ...
    for a in raw_assocs:
        ...
        other_id = str(a.target_entity_id) if str(a.source_entity_id) == str(entity_id) else str(a.source_entity_id)
