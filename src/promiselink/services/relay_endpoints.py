"""Relay endpoint methods for the RelayClient.

Contains :class:`RelayEndpointsMixin` — the four relay channels
(LLM / ASR / TTS / OCR), ``LLMProvider`` Protocol methods, and internal
HTTP helpers. Relies on the host :class:`RelayClient` for ``self.gateway_url``,
``self.timeout``, ``self._get_client()``, ``self._auth_headers()``,
``self._ensure_token()``, and ``self.refresh_token()``.
"""
...
__all__ = ["RelayEndpointsMixin"]


class RelayEndpointsMixin:
    """Relay channel endpoints and HTTP helpers (mixed into RelayClient)."""

    # Provided by the host class (RelayClient)
    gateway_url: str
    timeout: int
    max_retries: int
    _get_client: Any
