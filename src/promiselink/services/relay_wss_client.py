...
class RelayWSSClient:
    ...
    async def _handle_http_request(self, ws: Any, msg_data: dict[str, Any]) -> None:
        ...
        # Identity consumption (Gateway as Identity Broker, v0.9.7):
        # The gateway injects X-Local-User-Id into the WSS envelope so the
        # basic edition serves the same local_user data regardless of which
        # client (miniapp via relay, or local browser) originated the request.
        # The value travels inside the TLS WSS tunnel (not forgeable from
        # outside); we consume it as the local data-ownership identity.
        # The gateway's own Authorization/X-User-Token creds are never
        # forwarded to the local FastAPI.
        local_user_id = (
            headers.get("X-Local-User-Id")
            or headers.get("x-local-user-id")
            or "local_user"
        )
        local_token = create_access_token(local_user_id)
        forwarded_headers["Authorization"] = f"Bearer {local_token}"
