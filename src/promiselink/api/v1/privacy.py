...
from promiselink.models.voice_session import VoiceSession

logger = get_logger("promiselink.api.privacy")
router = APIRouter(prefix="/privacy", tags=["Privacy"], dependencies=[Depends(rate_limit_dependency)])
...
async def delete_user_data(
    ...
    # Delete in dependency order to respect foreign keys — all within the same session transaction
    # Note: get_async_session already provides a transaction context; we commit once at the end
